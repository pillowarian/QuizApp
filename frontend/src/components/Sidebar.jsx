...
const Sidebar = () => {
  ...
  const handleTechSelect = (techId) => {
    if (selectedTech === techId) {
      setSelectedTech(null); // all the initial values are defined there
  ...
  const resetQuiz = () =>{
    ...
    setUserAnswers({});
    setShowResults(false);
    const duration = selectedLevel === 'basic' ? 30 : selectedLevel === 'intermediate' ? 20 : 15;
  ...
  const getTimerDuration = () => {
    if (selectedLevel === 'basic') return 30;
    if (selectedLevel === 'intermediate') return 20;
    if (selectedLevel === 'advanced') return 15;
    return 30; // default
  };

  // Timer effect for each question
  useEffect(() => {
    if (!currentQ || userAnswers[currentQuestion] !== undefined || showResults) {
      if (timerRef.current) clearInterval(timerRef.current);
      return;
    }
  ...
  }, [currentQuestion, currentQ, userAnswers, showResults, questions.length, selectedLevel]);
  ...
  const submitResult = async () => {
    ...
    const payload = {
      ...
      wrong: (score.total - score.correct) || 0,
    };

    try{
        console.log("Submitting result...", payload);
        console.log("Auth token:", localStorage.getItem('authToken') ? 'Token exists' : 'No token');
        ...
        if(res.data && res.data.success) {
          ...
          console.error('Result save failed:', res.data);
                  ...
                  <div style={{display: 'flex', alignItems: 'center', gap: '1rem'}}>
                    ...
                    {userAnswers[currentQuestion] === undefined && (
                      <div style={{
                        padding: '0.5rem 1rem',
                        borderRadius: '9999px',
                        backgroundColor: timeLeft <= 10 ? '#fee2e2' : '#dbeafe',
